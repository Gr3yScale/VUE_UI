...
<template>
  <div class="flex flex-col gap-6 p-6 max-w-3xl mx-auto">
    ...
    <section class="flex flex-col gap-3">
      ...
      <div v-if="send.selectedName.value" class="flex items-center gap-2">
      ...
      <ul
        ...
        class="flex flex-col gap-1 max-h-48 overflow-y-auto rounded border border-compass-border bg-compass-surface p-1"
      >
        <li
          v-for="name in send.filteredNames.value"
          :key="name"
          :data-testid="`mq-named-sender-name-item-${name}`"
          class="cursor-pointer rounded px-3 py-1.5 text-sm text-compass-text hover:bg-compass-elevated"
          :class="{ 'bg-compass-elevated font-semibold': send.selectedName.value === name }"
