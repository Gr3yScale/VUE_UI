...
describe('QueryEditor', () => {
  ...
  it('renders all three textareas', () => {
    const wrapper = mount(QueryEditor, { props: defaultProps })
  ...
  it('run button is disabled when canRun is false', () => {
    const wrapper = mount(QueryEditor, { props: { ...defaultProps, canRun: false } })
    const btn = wrapper.find('[data-testid="query-editor-run-btn"]')
    expect((btn.element as HTMLButtonElement).disabled).toBe(true)
  })

  it('run button is disabled when isLoading', () => {
    const wrapper = mount(QueryEditor, { props: { ...defaultProps, isLoading: true } })
    const btn = wrapper.find('[data-testid="query-editor-run-btn"]')
    expect((btn.element as HTMLButtonElement).disabled).toBe(true)
  })
  ...
  it('shows filter error when filter is invalid', () => {
    const wrapper = mount(QueryEditor, { props: { ...defaultProps, filter: invalidState() } })
  ...
  it('shows sort error when sort is invalid', () => {
    const wrapper = mount(QueryEditor, { props: { ...defaultProps, sort: invalidState() } })
